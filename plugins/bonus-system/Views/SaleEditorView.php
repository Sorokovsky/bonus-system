...
class SaleEditorView
{
    public function render(SalesBonus $sale, int $index): void
    {
        ...
        <div class="bonus-tier-row" data-id="<?php echo $index; ?>">
            <input
                name="<?php echo SettingsService::OPTION_NAME;?>[sales][<?php echo $index; ?>][min_price]"
                value="<?php echo $sale->get_min_price(); ?>"
            ...
            <select name="<?php echo SettingsService::OPTION_NAME;?>[sales][<?php echo $index; ?>][discount_type]"
                    class="discount-type">
                <option value="<?php echo DiscountType::PERCENT->value; ?>"
                    <?php selected($sale->get_discount_type()->value, DiscountType::PERCENT->value); ?>>
                    % <?php _e("Відсоток", "bonus-system"); ?>
                </option>
                <option value="<?php echo DiscountType::FIXED->value; ?>"
                    <?php selected($sale->get_discount_type()->value, DiscountType::FIXED->value); ?>>
                    ₴ <?php _e("Фіксована", "bonus-system"); ?>
                </option>
            </select>
            <input type="number"
                   name="<?php echo SettingsService::OPTION_NAME;?>[sales][<?php echo $index; ?>][discount_value]"
                   value="<?php echo $sale->get_discount_amount(); ?>"
